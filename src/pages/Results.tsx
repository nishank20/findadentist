            ...
            <div className="lg:w-1/2 space-y-4 overflow-y-auto">
              ...
              {sortedDentists.map((dentist) => (
                <DentistCard
                  key={dentist.id}
                  dentist={dentist}
                  isHighlighted={highlightedDentistId === dentist.id}
                  onHighlight={() => {
                    setHighlightedDentistId(dentist.id);
                    setTimeout(() => setHighlightedDentistId(null), 3000);
                  }}
                  onBookAppointment={() => handleBookAppointment(dentist.name)}
                  showReviews={true}
                  onToggleReviews={() => toggleReviews(dentist.id)}
                  reviewsExpanded={expandedReviews.includes(dentist.id)}
                  cardRef={(el) => dentistRefs.current[dentist.id] = el}
                />
              ))}
              </div>
            </div>
